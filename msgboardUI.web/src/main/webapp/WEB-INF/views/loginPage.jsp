<%@ page import="com.vgtech.platform.common.utility.VGUtility" %>
...
<script type="text/javascript">
	...
	$(function() {
		...
		if (isError()) {
			var flag = $("#eMessage").val();
			if (flag != null && $.trim(flag) != "") {
				$('#loginForm .eMessage').html(flag);
				$('#loginForm .eMessage').show();
			} else {
				$('#loginForm .error').html("密码错误！您今天一共有五次机会连续输错密码。");
			...
			if (strs[0] == 'error') {
				if (strs[1] != null && strs[1] != "") {
					if (isNaN(strs[1])) {
						...
						$("#eMessage").val("您已经连续输错五次密码！请" + strs[1] + "分钟后重新登录！");
					}
				}
				return true;
			} else {
	...
	function registe() {
		...
		if (!usernameReg.test(username)) {
			...
			alert("密码请输入8-40位以上的数字字母或者英文字符组合");
...
<%--						<label class="error" style="display: none; color: #e2161d">用户或密码错误</label>--%>
						<label class="error" style="display: none; color: #e2161d"></label>
						<label class="eMessage" style="display: none; color: #e2161d"></label>
						<!--                 <p class="forgot"><a onclick="forgotPwd()">忘记密码?</a></p> -->
						<div class="rem_sub" style="margin-top: 40px;">
							<div>
								<button id="loginButton" class="sub_button" onclick="login()" style="cursor:pointer">登&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;陆</button>
