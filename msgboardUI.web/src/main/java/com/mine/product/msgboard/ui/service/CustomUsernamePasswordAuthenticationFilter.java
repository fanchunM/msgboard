...
public class CustomUsernamePasswordAuthenticationFilter extends AbstractAuthenticationProcessingFilter {
	...
	private boolean validate(String username, String password) {
		...
		public Authentication attemptAuthentication(HttpServletRequest request,
			...
			try {
				username = new String(obtainUsername(request).getBytes("ISO8859-1"),"UTF-8");
			...
			boolean flag = personService.loginJudge(username);
			if (!flag) {
				...
				throw new InternalAuthenticationServiceException(String.valueOf(sessionTime/60));
			} else if (flag) {
				boolean result = this.validate(username, password);
				if (!result) {
					//添加登录失败的信息
					personService.addLoginHistory(username, 0, IPUtils.getClientIp(request));
				} else {
					//添加登录成功的信息
					personService.addLoginHistory(username, 1, IPUtils.getClientIp(request));
				}
			}
