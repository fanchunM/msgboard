...
function send(){
	...
	var radios=document.getElementsByName("attribute1");
	for(var i=0;i<radios.length;i++)
	{
		if(radios[i].checked==true)
		{
			attribute1 = radios[i].value;
		}
	}
	...
	if($('#validateCode').val() == ''
		...
		$.ajax({
				url : '/admin/newsMessage/create.do',
				type : 'post',
				data : inputForm,
				dataType : 'json',
...
function changeCode() {
	$('#randomImage').attr('src', '/admin/newsMessage/validateCode.do' + '?timestamp=' + (new Date()).valueOf());
	$('#validateCode').focus();
};
...
function closeDL(){
	...
	document.getElementById("top-festival").style.display='none';
